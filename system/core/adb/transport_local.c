...
//	fd=socket_bluetooth_client(server_addr);
	//fd=socket_bluetooth_client(host, adb_port, SOCK_STREAM);
	if (fd >= 0) {
...
//add bluetooth connect to the remote
int sk;//connection num
 int i = 0;
 char writebuff[128];//buffer for write
 char sendbuff[128]="cmd:";//buffer for send
 struct sockaddr_l2 remote_addr;
 int iRel = 0;
 pthread_t nth=0;//read thread
 ...
 str2ba("C8:A0:30:75:5C:30", &remote_addr.l2_bdaddr);
 remote_addr.l2_psm = htobs(0x1001);
 
 connect(sk, (struct sockaddr*)&remote_addr, sizeof(remote_addr));
...
//adb初次启动服务时，搜寻本机上Android模拟器的adbd服务
    ...
    for ( ; count > 0; count--, port += 2 ) {
		...
		fprintf(stdout,"尝试连接android模拟器端口号%d\n",port);
    }
